...
import { PieChart as RechartsPieChart, Cell, ResponsiveContainer, BarChart, Bar, XAxis,
YAxis, CartesianGrid, Tooltip, LineChart, Line, Legend, Pie } from 'recharts';
...
const [creditBillAmount, setCreditBillAmount] = useState(0); // Total amount spent on
credit card
...
expenses.filter(e => e.paymentMethod === 'salary').reduce((sum, expense) => sum +
expense.amount, 0),
[expenses]
);
const creditExpenses = React.useMemo(() =>
expenses.filter(e => e.paymentMethod === 'credit').reduce((sum, expense) => sum +
expense.amount, 0),
[expenses]
);const totalExpenses = React.useMemo(() => salaryExpenses + creditExpenses,
[salaryExpenses, creditExpenses]);
// Calculate financial distribution
const { remainingSalary, availableCredit, totalDebt, creditBill } = React.useMemo(() =>
{
const currentSalaryUsed = salaryExpenses + creditBillAmount; // Salary used includes
bill payments
const currentCreditBill = creditExpenses; // Bill amount is credit expenses minus
payments
const currentCreditUsed = currentCreditBill; // Credit used is the bill amount
const currentDebt = Math.max(0, currentCreditUsed - creditLimit); // Debt if exceeds
credit limit
return {
...
}, [salaryExpenses, creditExpenses, creditBillAmount, salary, creditLimit,
totalExpenses]);
...
alert(`
Você ultrapassou o limite do cartão. Agora está devendo ao banco R$
${financialBreakdown.debt.toFixed(2)}!`);
...
const users = JSON.parse(localStorage.getItem('budgetProUsers') || '[]');
const user = users.find((u: any) => u.email === resetEmail);if (user) {
...
alert(`O valor do pagamento não pode ser maior que a fatura (R$
${creditBill.toFixed(2)}).`);
return;
}
if (paymentAmount > remainingSalary) {
alert(`Você não tem saldo suficiente no salário (R$
...
✅
alert(`
Pagamento de R$ ${paymentAmount.toFixed(2)} realizado com sucesso!`);
...
if (amount < selectedInvestment.minInvestment || amount >
selectedInvestment.maxInvestment) {
alert(`Valor deve estar entre R$ ${selectedInvestment.minInvestment} e R$
${selectedInvestment.maxInvestment}.`);
return;
}
...
const returnPercentage = (selectedInvestment.expectedReturn / 100) * variationFactor *
0.5; // Half the expected range for realism
...
useEffect(() => {
const currentMonth = new Date().toLocaleString("pt-BR", { month: "short" }); // ex:
"nov"
...
color: availableSalary >= 0 ? '#046BF4' : '#FF4C4C', // Azul se positivo, vermelho se
negativo
...
color: '#8B5CF6', // Roxo
percentage: ((financialBreakdown.creditUsed / (salary + creditLimit)) *
100).toFixed(1)
});
}
// Salário usado (verde)
...
color: '#10B981', // Verde
percentage: ((financialBreakdown.salaryUsed / (salary + creditLimit)) *
100).toFixed(1)
});
}
...
return data;
}, [remainingSalary, totalDebt, financialBreakdown, availableCredit, salary,
creditLimit]);
...
src={logoDefinitiva}
alt="BudgetPro Logo"
className="w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 lg:w-48 lg:h-48 object-
contain"
/>
</div>
{/* Loading animation - Mobile First */}
<div className="relative w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 flex items-
...
<h1 className="text-white text-3xl md:text-4xl lg:text-5xl mb-2 font-
bold">BudgetPro</h1>
...
<h2 className="text-xl md:text-2xl lg:text-3xl mb-2 text-white font-
semibold">
Bem-vindo de volta!
</h2>
...
className="pl-11 h-12 rounded-xl border-gray-200 focus:border-sky-
400 focus:ring-2 focus:ring-sky-200 transition-all"
/>
</div>
<div className="relative">
...
hover:shadow-xl transition-all duration-200 transform hover:scale-105 hover:brightness-
110"
style={{ backgroundColor: '#046BF4' }}
>
Entrar
...
return (
<motion.divclassName="min-h-screen"
style={{ background: 'linear-gradient(135deg, #046BF4 0%, #2A9DF4 50%, white
100%)' }}
initial={{ opacity: 0 }}
animate={{ opacity: 1 }}
transition={{ duration: 0.6 }}
...
{/* Header - Mobile First */}
<div className="px-4 py-8 text-center">
<h1 className="text-3xl md:text-4xl lg:text-5xl mb-2 font-bold" style={{ color:
'#046BF4' }}>BudgetPro</h1>
<h2 className="text-lg md:text-2xl lg:text-3xl text-gray-700 md:text-white/90
font-semibold">Criar Nova Conta</h2>
...
return (
<motion.divclassName="min-h-screen bg-gray-50"
initial={{ opacity: 0 }}
animate={{ opacity: 1 }}
transition={{ duration: 0.6 }}
>
{/* Header - Mobile First */}
<div className="px-4 py-8 text-center">
<h1 className="text-3xl mb-2 font-bold" style={{ color: '#046BF4'
}}>BudgetPro</h1>
...
className="pl-11 h-12 rounded-xl border-gray-200 focus:border-sky-400
focus:ring-2 focus:ring-sky-200 transition-all"
/>
</div>
<Button
...
<p className="text-sm font-semibold">R$ {(remainingSalary +
availableCredit).toFixed(2)}</p>
</div>
</div>
</div>
...
Saldo baixo! Restam R$ {remainingSalary.toFixed(2)}
</AlertDescription>
</Alert>
</motion.div>
)}
...
<TabsList className="grid w-full grid-cols-2 md:grid-cols-4 rounded-xl p-1 h-
auto md:h-11" style={{ backgroundColor: '#f8fafc' }}>
<TabsTrigger
value="overview"
className="rounded-lg data-[state=active]:bg-[#046BF4] data-
[state=active]:shadow-sm data-[state=active]:text-white hover:bg-sky-100 transition-all
...
{/* Income Card - Mobile */}
<Card className="shadow-md border-0 rounded-xl hover:shadow-lg transition-
shadow">
...
<div>
<p className="text-lg font-semibold text-green-600">R$
{salary.toFixed(2)}</p>
...
<span className="text-xs text-gray-500 uppercase">Gastos
Salário</span>
</div>
<div>
<p className="text-lg font-semibold text-red-600">R$
{salaryExpenses.toFixed(2)}</p>
...
<div><p className="text-lg font-semibold" style={{ color: '#046BF4' }}>
...
<div>
<p className="text-lg font-semibold text-purple-600">R$
{availableCredit.toFixed(2)}</p>
...
<span className="text-sm">Você gastou
{Math.round(expensePercentage)}% da sua renda</span>
<span className="text-sm font-semibold">
{Math.round(expensePercentage)}%</span>
</div>
<Progress
value={expensePercentage}
className="h-3 rounded-full"
style={{
'--progress-background': expensePercentage > 80 ? '#EF4444' :
expensePercentage > 60 ? '#F59E0B' : '#046BF4'
...
✅
🚨
⚠️{/* Progress Ring - Mobile Center */}
<div className="flex justify-center mt-4">
<ProgressRing
percentage={expensePercentage}
size={100}
color={expensePercentage > 80 ? '#EF4444' : expensePercentage > 60 ?
'#F59E0B' : '#046BF4'}
/>
</div>
</CardContent>
...
size="sm"
className="flex-1 rounded-lg hover:brightness-110 transition-
all"
style={{ backgroundColor: '#046BF4' }}
>
Salvar
...
<>
<div className="mb-3">
<p className="text-xl font-semibold text-purple-600">R$
{creditLimit.toFixed(2)}</p>
...
variant="outline"
className="w-full rounded-lg border-2 hover:bg-sky-50
...
Gastos</h2>
📋 Pranchetas de
<p className="text-sm text-gray-600">Gerencie seus gastos de salário e
cartão de crédito</p></div>
{/* Side by Side Boards - Responsive */}
<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
{/* LEFT BOARD: SALARY EXPENSES */}
<div className="space-y-4">
{/* Summary Card - Salary */}
<Card className="shadow-md border-0 rounded-xl bg-gradient-to-br from-
green-50 to-blue-50">
...
Salário Total</span>
<span className="font-semibold text-green-600">R$
{salary.toFixed(2)}</span>
</div>
<div className="flex items-center justify-between">
<span className="text-sm text-gray-700">
Total Gasto</span>
<span className="font-semibold text-red-600">R$
{salaryExpenses.toFixed(2)}</span>
</div>
...
Saldo
Restante</span>
<span className="font-bold text-lg" style={{ color: '#046BF4'
}}>R$ {remainingSalary.toFixed(2)}</span>
</div>
</div>
</CardContent>
</Card>
...
percentage={expensePercentage}
size={80}
color={expensePercentage > 80 ? '#EF4444' : expensePercentage >
60 ? '#F59E0B' : '#046BF4'}
/>
...
<div className="h-24">
<ResponsiveContainer width="100%" height="100%"><RechartsPieChart>
<Pie
...
<div className="h-24 flex items-center justify-center text-gray-
400 text-xs">
Sem dados
</div>
)}
...
<div className="space-y-3 mb-4 p-3 bg-green-50 rounded-lg border
💵
border-green-200">
<p className="text-xs text-gray-700 font-medium">Adicionar novo
gasto</p>
<Input
...
className="rounded-lg px-4 h-10 hover:brightness-110
transition-all"
style={{ backgroundColor: '#046BF4' }}
>
...
<div>
<span className="font-medium text-sm">{expense.category}
</span>
<p className="text-xs text-gray-500">
{salaryExpenses > 0 ? ((expense.amount /
salaryExpenses) * 100).toFixed(1) : '0'}% dos gastos
</p>
</div>
</div>
...
<span className="text-sm text-gray-700">
Limite Total</span>
<span className="font-semibold text-purple-600">R$
{creditLimit.toFixed(2)}</span>
</div>
<div className="flex items-center justify-between">
<span className="text-sm text-gray-700">
Valor Gasto</span>
<span className="font-semibold text-red-600">R$
{creditExpenses.toFixed(2)}</span>
</div>
<div className="h-px bg-gray-300 my-2"></div>
<div className="flex items-center justify-between">
<span className="text-sm font-medium text-gray-800">
Limite
Restante</span>
<span className="font-bold text-lg text-purple-600">R$
{availableCredit.toFixed(2)}</span>
</div>
</div>
</CardContent>
</Card>
...
size={80}
color={creditPercentage > 80 ? '#EF4444' : creditPercentage > 60
? '#F59E0B' : '#8B5CF6'}
/>
<p className="text-xs text-gray-500 mt-2 text-center">
...
<div className="space-y-3 mb-4 p-3 bg-purple-50 rounded-lg border
border-purple-200">
<p className="text-xs text-gray-700 font-medium">Adicionar novo
gasto</p>
<Input
...
className="rounded-lg px-4 h-10 hover:brightness-110
transition-all bg-purple-600 hover:bg-purple-700"
>
<PlusCircle className="w-4 h-4" />
</Button>
</div>
...
<p className="text-xs text-gray-500">
{creditExpenses > 0 ? ((expense.amount /
creditExpenses) * 100).toFixed(1) : '0'}% da fatura
</p>
</div>
</div>
<div className="flex items-center gap-2">
...
{/* Bill Summary Card */}
<Card className="shadow-md border-0 rounded-xl bg-gradient-to-br from-blue-
50 to-purple-50">
<CardHeader>
<CardTitle className="flex items-center gap-2 text-sm">
<div className="p-2 rounded-lg" style={{ backgroundColor: '#046BF4'
...
<div className="space-y-3 p-4 bg-white rounded-lg border-2 border-
purple-200">
<div className="flex items-center justify-between">
<span className="text-sm text-gray-700">
Valor da Fatura
Atual</span>
<span className="font-bold text-lg text-purple-600">R$
{creditBill.toFixed(2)}</span>
</div>
<div className="flex items-center justify-between">
<span className="text-sm text-gray-700">
Já Pago</span>
<span className="font-semibold text-green-600">R$
{creditBillAmount.toFixed(2)}</span>
</div>
...
Saldo disponível no salário: <strong>R$
{remainingSalary.toFixed(2)}</strong>
</AlertDescription>
</Alert>
💰
<div className="space-y-3 p-4 bg-white rounded-lg border border-
gray-200">
...
<div className="flex gap-2">
<Button
onClick={() => setBillPaymentAmount((creditBill -
creditBillAmount).toFixed(2))}
variant="outline"
...
className="w-full h-12 rounded-lg text-white font-medium shadow-
...
Fatura Paga!</p>
<p className="text-sm text-gray-600 mt-1">Você não tem saldo devedor
✅
no momento.</p>
</div>
)}
</CardContent>
</Card>{/* Payment Info Card */}
<Card className="shadow-md border-0 rounded-xl">
<CardHeader>
<CardTitle className="text-sm">
Como Funciona</CardTitle>
</CardHeader>
<CardContent className="space-y-2 text-xs text-gray-600">
<p>• O pagamento da fatura sai do <strong>saldo do seu salário</strong>
ℹ️
</p>
<p>• Após o pagamento, o <strong>limite do cartão é liberado</strong>
</p>
...
<CardContent className="space-y-4">
<div className="p-3 rounded-lg bg-gradient-to-r from-blue-50 to-indigo-
50 border border-blue-200">
<div className="flex items-start gap-3 mb-3">
<TrendingUp className="w-4 h-4 text-blue-600 mt-1" />
<div>
<h4 className="font-medium text-blue-900 text-sm">Sugestões
Personalizadas</h4>
<p className="text-xs text-blue-700 mt-1">
Com saldo de R$ {remainingSalary.toFixed(2)}, recomendamos
...
<Alert className="border-orange-200 bg-orange-50 rounded-xl"><AlertTriangle className="h-4 w-4 text-orange-600" />
<AlertDescription className="text-orange-800 text-xs">
<strong>Aviso:</strong> Investimentos envolvem riscos. Pode haver
perda total.
📊
...
<div className="mt-6">
<h2 className="text-lg font-semibold text-gray-800 mb-3">Distribuição de
Investimentos</h2>
<Card className="shadow-md border-0 rounded-xl">
...
<Cell
key={`cell-${index}`}
fill={['#4F46E5', '#22C55E', '#F59E0B', '#EF4444', '#06B6D4'][index %
5]}
/>
))}
</Pie>
...
<div><h2 className="text-xl font-semibold">{selectedInvestment.name}</h2>
<p className="text-gray-600">{selectedInvestment.type}</p>
<span
className="inline-block text-xs px-2 py-1 rounded-full mt-1"
style={{
backgroundColor:
`${getRiskColor(selectedInvestment.riskLevel)}20`,
...
<strong>Aviso de risco:</strong> Esse investimento pode gerar perda
total. Invista com cautela.
Os valores apresentados são simulações e não garantem rendimentos
futuros.
</AlertDescription>
</Alert>
{/* Action Buttons */}
...
<Button
onClick={() => setCurrentScreen('investment-purchase')}className="flex-1 h-12 rounded-2xl text-white shadow-lg hover:shadow-xl
transition-all duration-200 transform hover:scale-105 hover:brightness-110"
style={{ backgroundColor: '#046BF4' }}
>
Investir Agora
</Button>
...
onClick={() => setCurrentScreen('dashboard')}
variant="outline"
className="flex-1 h-12 rounded-2xl border-2 hover:bg-sky-50 transition-
all"
style={{ borderColor: '#046BF4', color: '#046BF4' }}
>
Voltar ao Dashboard
